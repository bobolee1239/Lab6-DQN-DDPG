...
class DQN:
    ...
    def select_action(self, state, epsilon, action_space):
        ...
        if random.random() < epsilon:
            ...
            return self.select_best_action(state).item()
    ...
    def _update_behavior_network(self, gamma):
        ...
        with torch.no_grad():
            action_est = self.select_best_action(next_state)         # (N, 1)
            qs_next     = self._target_net(next_state)               # (N, act_dim)
            q_next = torch.gather(input=qs_next, dim=-1, index=action_est.long())
